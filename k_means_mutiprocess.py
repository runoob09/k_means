...
class KMeans:
    def __init__(self, n_clusters, n_processes=2, max_iterations=100):
        # 簇数
    ...
    def train(self, X):
        ...
        with Pool(processes=self.n_process) as pool:
            for _ in range(self.max_iterations):
                ...
                group = init_group()
                # 生成任务
                tasks = [(choose_near_point, x, self.centroids) for x in X]
                result = pool.map(wrapper, tasks)
                for idx, point in result:
