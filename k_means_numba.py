...
@cuda.jit
def compute_mean_gpu(vectors, result):
    # 获取当前线程的索引
    i = cuda.grid(1)
    # 初始化求和变量
    sum_vector = 0.0
    ...
    for j in range(vectors.shape[1]):
        sum_vector += vectors[i, j]
    # 计算均值
    result[i] = sum_vector / vectors.shape[1]
...
def mean_v(vectors):
    ...
    # 创建结果数组
    result = cuda.device_array((vectors.shape[1],), dtype=vectors.dtype)
    # 配置 CUDA 栅格和线程块
    threads_per_block = 1
    blocks_per_grid = (vectors.shape[0] + (threads_per_block - 1)) // threads_per_block
    # 调用 CUDA 函数计算向量均值
    compute_mean_gpu[blocks_per_grid, threads_per_block](d_vectors, result)
    # 将结果移回到主机内存
    result_host = result.copy_to_host()
    return result_host
